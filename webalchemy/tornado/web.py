...
"""``webalchemy.tornado.web`` provides a simple web framework with asynchronous
...
from webalchemy.tornado.concurrent import Future
from webalchemy.tornado import escape
from webalchemy.tornado import httputil
from webalchemy.tornado import locale
from webalchemy.tornado.log import access_log, app_log, gen_log
from webalchemy.tornado import stack_context
from webalchemy.tornado import template
from webalchemy.tornado.escape import utf8, _unicode
from webalchemy.tornado.util import bytes_type, import_object, ObjectDict, raise_exc_info, unicode_type
...
class RequestHandler(object):
    ...
    def clear(self):
        ...
        self._headers = httputil.HTTPHeaders({
            "Server": "TornadoServer/%s" % webalchemy.tornado.version,
    ...
    def get_template_namespace(self):
        ...
        defaults in the `webalchemy.tornado.template` module and keyword arguments
    ...
    def get_user_locale(self):
        ...
        This method should return a `webalchemy.tornado.locale.Locale` object,
        most likely obtained via a call like ``webalchemy.tornado.locale.get("en")``
        """
        return None
    ...
    def get_login_url(self):
        ...
        """
        self.require_setting("login_url", "@webalchemy.tornado.web.authenticated")
    ...
    def compute_etag(self):
        ...
        or may return None to disable webalchemy.tornado's default etag support.
        """
        hasher = hashlib.sha1()
    ...
    def log_exception(self, typ, value, tb):
        ...
        stack traces (on the ``webalchemy.tornado.general`` logger), and all
        other exceptions as errors with stack traces (on the
        ``webalchemy.tornado.application`` logger).
...
class Application(object):
    ...
    def __init__(self, handlers=None, default_host="", transforms=None,
        ...
        if self.settings.get('autoreload') and not wsgi:
            from webalchemy.tornado import autoreload
            autoreload.start()
...
class FallbackHandler(RequestHandler):
    ...
    `webalchemy.tornado.wsgi.WSGIContainer`.  This is most useful to use both
    Tornado ``RequestHandlers`` and WSGI in the same server.  Typical
    usage::

        wsgi_app = webalchemy.tornado.wsgi.WSGIContainer(
