...
the `webalchemy.tornado.web.RequestHandler` class.  They are used in two ways:
...
from webalchemy.tornado.concurrent import TracebackFuture, chain_future, return_future
from webalchemy.tornado import gen
from webalchemy.tornado import httpclient
from webalchemy.tornado import escape
from webalchemy.tornado.httputil import url_concat
from webalchemy.tornado.log import gen_log
from webalchemy.tornado.util import bytes_type, u, unicode_type, ArgReplacer
...
def _auth_return_future(f):
    """Similar to webalchemy.tornado.concurrent.return_future, but uses the auth
...
class TwitterMixin(OAuthMixin):
    ...
    `~webalchemy.tornado.web.Application.settings` ``twitter_consumer_key`` and
...
class FriendFeedMixin(OAuthMixin):
    ...
    `~webalchemy.tornado.web.Application.settings` ``friendfeed_consumer_key``
...
class FacebookMixin(object):
    ...
    *Deprecated:* New applications should use `FacebookGraphMixin`
    ...
    to authenticate the user with Facebook::
        ...
        class FacebookHandler(webalchemy.tornado.web.RequestHandler,
                              webalchemy.tornado.auth.FacebookMixin):
            ...
            def _on_auth(self, user):
                if not user:
                    raise webalchemy.tornado.web.HTTPError(500, "Facebook auth failed")
                # Save the user using, e.g., set_secure_cookie()
