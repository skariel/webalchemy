...
* `WSGIApplication` is a version of `webalchemy.tornado.web.Application` that can run
...
from webalchemy.tornado.log import access_log
from webalchemy.tornado import web
from webalchemy.tornado.escape import native_str, parse_qs_bytes
from webalchemy.tornado.util import bytes_type, unicode_type
...
# These functions are like those in the webalchemy.tornado.escape module, but defined
...
class WSGIApplication(web.Application):
    """A WSGI equivalent of `webalchemy.tornado.web.Application`.

    `WSGIApplication` is very similar to `webalchemy.tornado.web.Application`,
    ...
    Example usage::
        ...
        if __name__ == "__main__":
            application = webalchemy.tornado.wsgi.WSGIApplication([
                (r"/", MainHandler),
            ])
    ...
    <https://github.com/facebook/webalchemy.tornado/tree/master/demos/appengine>`_
    ...
    not possible to use `.AsyncHTTPClient`, or the `webalchemy.tornado.auth` or
    `webalchemy.tornado.websocket` modules.
    """
...
class HTTPRequest(object):
    """Mimics `webalchemy.tornado.httpserver.HTTPRequest` for WSGI applications."""
...
class WSGIContainer(object):
    ...
    run it. For example::
        ...
        container = webalchemy.tornado.wsgi.WSGIContainer(simple_app)
        http_server = webalchemy.tornado.httpserver.HTTPServer(container)
    ...
    The `webalchemy.tornado.web.FallbackHandler` class is often useful for mixing
    Tornado and WSGI apps in the same server.  See
    https://github.com/bdarnell/django-webalchemy.tornado-demo for a complete example.
    """
    ...
    @staticmethod
    def environ(request):
        """Converts a `webalchemy.tornado.httpserver.HTTPRequest` to a WSGI environment.
        """
