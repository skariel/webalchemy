...
`webalchemy.tornado.web.Application` constructor (or ``debug=True``, which
...
# path.  We attempt to address the latter problem when webalchemy.tornado.autoreload
...
if __name__ == "__main__":
    ...
    # as possible - if we introduced a webalchemy.tornado.sys or webalchemy.tornado.os
...
from webalchemy.tornado import ioloop
from webalchemy.tornado.log import gen_log
from webalchemy.tornado import process
from webalchemy.tornado.util import exec_in
...
def start(io_loop=None, check_time=500):
    ...
    if len(_io_loops) > 1:
        gen_log.warning("webalchemy.tornado.autoreload started more than once in the same process")
...
def add_reload_hook(fn):
    ...
    ``webalchemy.tornado.platform.auto.set_close_exec``) instead
...
_USAGE = """\
Usage:
  python -m webalchemy.tornado.autoreload -m module.to.run [args...]
  python -m webalchemy.tornado.autoreload path/to/script.py [args...]
"""
