...
from webalchemy.tornado.netutil import ssl_wrap_socket, ssl_match_hostname, SSLCertificateError
from webalchemy.tornado import stack_context
from webalchemy.tornado.util import bytes_type

try:
    from webalchemy.tornado.platform.posix import _set_nonblocking
except ImportError:
...
class IOStream(BaseIOStream):
    ...
    A very simple (and broken) HTTP client using this class::
        ...
        def on_body(data):
            print data
            stream.close()
            webalchemy.tornado.ioloop.IOLoop.instance().stop()

        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM, 0)
        stream = webalchemy.tornado.iostream.IOStream(s)
