...
* `main()`: A simple test runner (wrapper around unittest.main()) with support
  for the webalchemy.tornado.autoreload module to rerun the tests when code changes.
"""
...
try:
    ...
    from webalchemy.tornado.httpserver import HTTPServer
    from webalchemy.tornado.simple_httpclient import SimpleAsyncHTTPClient
    from webalchemy.tornado.ioloop import IOLoop
    from webalchemy.tornado import netutil
except ImportError:
...
from webalchemy.tornado.log import gen_log
from webalchemy.tornado.stack_context import ExceptionStackContext
from webalchemy.tornado.util import raise_exc_info, basestring_type
import functools
import logging
import os
import re
...
# (either py27+ or unittest2) so webalchemy.tornado.test.util enforces
# this requirement, but for other users of webalchemy.tornado.testing we want
...
class AsyncTestCase(unittest.TestCase):
    ...
    used with the `webalchemy.tornado.gen` module, decorate your test methods
    with `webalchemy.tornado.testing.gen_test` instead of
    `webalchemy.tornado.gen.coroutine`.  This class also provides the `stop()`
    ...
    Example::
        ...
        class MyTestCase(AsyncTestCase):
            @webalchemy.tornado.testing.gen_test
...
class AsyncHTTPTestCase(AsyncTestCase):
    ...
    `webalchemy.tornado.web.Application` (or other `.HTTPServer` callback) to be tested.
    ...
    def get_app(self):
        ...
        `webalchemy.tornado.web.Application` or other `.HTTPServer` callback.
        """
...
class AsyncHTTPSTestCase(AsyncHTTPTestCase):
    ...
    def get_ssl_options(self):
        ...
        # openssl req -new -keyout webalchemy.tornado/test/test.key -out webalchemy.tornado/test/test.crt -nodes -days 3650 -x509
...
class LogTrapTestCase(unittest.TestCase):
    ...
    `webalchemy.tornado.options`.  It is not compatible with other log buffering
...
class ExpectLog(logging.Filter):
    ...
    Usage::
        ...
        with ExpectLog('webalchemy.tornado.application', "Uncaught exception"):
