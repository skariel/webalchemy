...
(and even that is often done indirectly via `webalchemy.tornado.web.Application.listen`).

This module also defines the `HTTPRequest` class which is exposed via
`webalchemy.tornado.web.RequestHandler.request`.
"""
...
from webalchemy.tornado import iostream
from webalchemy.tornado.log import gen_log
from webalchemy.tornado import netutil
from webalchemy.tornado.tcpserver import TCPServer
from webalchemy.tornado import stack_context
from webalchemy.tornado.util import bytes_type
...
class HTTPServer(TCPServer):
    ...
    requested::
        ...
        http_server = webalchemy.tornado.httpserver.HTTPServer(handle_request)
    ...
    initialization methods are defined on `webalchemy.tornado.tcpserver.TCPServer`):

    1. `~webalchemy.tornado.tcpserver.TCPServer.listen`: simple single-process::

            server = HTTPServer(app)
            server.listen(8888)
            IOLoop.instance().start()

       In many cases, `webalchemy.tornado.web.Application.listen` can be used to avoid
       the need to explicitly create the `HTTPServer`.

    2. `~webalchemy.tornado.tcpserver.TCPServer.bind`/`~webalchemy.tornado.tcpserver.TCPServer.start`:
    ...
    3. `~webalchemy.tornado.tcpserver.TCPServer.add_sockets`: advanced multi-process::

            sockets = webalchemy.tornado.netutil.bind_sockets(8888)
            webalchemy.tornado.process.fork_processes(0)
            server = HTTPServer(app)
            server.add_sockets(sockets)
            IOLoop.instance().start()

       The `~.TCPServer.add_sockets` interface is more complicated,
       but it can be used with `webalchemy.tornado.process.fork_processes` to
       give you more flexibility in when the fork happens.
       `~.TCPServer.add_sockets` can also be used in single-process
       servers if you want to create your listening sockets in some
       way other than `webalchemy.tornado.netutil.bind_sockets`.
