...
from webalchemy.tornado.util import u, unicode_type
...
class TranslationLoaderTest(unittest.TestCase):
    ...
    def clear_locale_cache(self):
        if hasattr(webalchemy.tornado.locale.Locale, '_cache'):
            del webalchemy.tornado.locale.Locale._cache

    def setUp(self):
        self.saved = {}
        for var in TranslationLoaderTest.SAVE_VARS:
            self.saved[var] = getattr(webalchemy.tornado.locale, var)
        self.clear_locale_cache()

    def tearDown(self):
        for k, v in self.saved.items():
            setattr(webalchemy.tornado.locale, k, v)
        self.clear_locale_cache()
...
class EnglishTest(unittest.TestCase):
    def test_format_date(self):
        locale = webalchemy.tornado.locale.get('en_US')
